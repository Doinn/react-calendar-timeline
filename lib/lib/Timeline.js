...
function (_Component) {
  ...
  function ReactCalendarTimeline(_props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "updateVirtualRowRange", function () {
      if (!_this.container || !_this.state.groupHeights) return; // eslint-disable-next-line global-require

      var _require = require('./utility/rowVirtualization'),
          buildPrefixSum = _require.buildPrefixSum,
          findVisibleRange = _require.findVisibleRange;

      var rect = _this.container.getBoundingClientRect();

      var viewportHeight = window.innerHeight;
      var calendarTop = rect.top + window.scrollY;
      var scrollTop = Math.max(0, window.scrollY - calendarTop);
      var heights = _this.state.groupHeights;
      var prefixSum = buildPrefixSum(heights);

      var _findVisibleRange = findVisibleRange({
        prefixSum: prefixSum,
        scrollTop: scrollTop,
        viewportHeight: viewportHeight,
        overscan: 5
      }),
          first = _findVisibleRange.first,
          last = _findVisibleRange.last;

      if (first !== _this.state.visibleRowFirst || last !== _this.state.visibleRowLast || prefixSum !== _this.state.groupTopsPrefixSum) {
        _this.setState({
    ...
    _this.state.visibleRowFirst = -1;
    _this.state.visibleRowLast = -1;
    _this.state.groupTopsPrefixSum = [0];
    /* eslint-enable */
  ...
  _createClass(ReactCalendarTimeline, [{
    ...
    value: function componentDidMount() {
      ...
      this._onWindowScroll = function () {
        if (_this3._rafScrollPending) return;
        _this3._rafScrollPending = true;
        requestAnimationFrame(function () {
          _this3._rafScrollPending = false;

          _this3.updateVirtualRowRange();
        });
      };
    ...
    key: "rows",
    value: function rows(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, canvasWidth, groupHeights, groups) {
      return _react["default"].createElement(_GroupRows["default"], {
        ...
        horizontalLineClassNamesForGroupCell: this.props.horizontalLineClassNamesForGroupCell,
    ...
    value: function render() {
      ...
      var virtFirst = isInteractingWithItem ? 0 : this.state.visibleRowFirst;
      var virtLast = isInteractingWithItem ? groups.length - 1 : this.state.visibleRowLast;
