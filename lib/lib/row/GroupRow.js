...
function (_Component) {
  ...
  _createClass(GroupRow, [{
    ...
    value: function render() {
      ...
      if (groupIdCells.includes(group.id)) (0, _calendar.iterateTimes)(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, function (time, nextTime) {
        var left = getLeftOffsetFromDate(time.valueOf());
        var right = getLeftOffsetFromDate(nextTime.valueOf());
        var width = right - left;
        var classNamesForGroupCell = [];
        ...
        cells.push(_react["default"].createElement("div", {
          ...
          className: "line-".concat(time.valueOf(), " ") + (classNamesForGroupCell ? classNamesForGroupCell.join(' ') : ''),
          style: {
            position: 'absolute',
            pointerEvents: 'none',
            top: '1px',
            left: "calc(".concat(left, "px + 1px)"),
            width: "calc(".concat(width, "px - 1px)"),
            height: "calc(".concat(style.height, " - 1px)")
          }
        }));
      });
